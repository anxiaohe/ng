...
import { curYear, SourceType, treeDataSource } from './data';
...
export class HomeAComponent implements OnInit, AfterViewInit, OnDestroy {
  curYear = curYear;
  ...
  ngOnInit() {
    ...
    this.ganttSacleConfigHandler = this.ganttService.ganttScaleConfigChange.subscribe((config) => {
      ...
      if (config.startDate || config.endDate) {
        this.ganttScaleWidth = this.ganttService.getDurationWidth(this.ganttStartDate, this.ganttEndDate) + 'px';
        const len = (this.tableWidthConfig as any[]).length;
  ...
  onResized(event: any) {
    ...
    const index = (this.tableWidthConfig as any[]).findIndex((i) => i.field === 'status');
