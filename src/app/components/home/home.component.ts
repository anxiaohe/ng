...
import { basicData, curYear } from './data';
...
export class HomeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  isFullScreen = false;
  scaleStep = 50;
  private mouseDownHandler!: Subscription;
  private mouseMoveHandler!: Subscription;
  private mouseEndHandler!: Subscription;
  ...
  onSwitchView(unit: any) {
    this.unit = unit;
    this.ganttService.setScaleConfig({ unit });
    this.ganttScaleWidth = this.ganttService.getDurationWidth(this.ganttStartDate, this.ganttEndDate) + 'px';
  }

  launchFullscreen({ isFullscreen }: any) {
  ...
  ngAfterViewInit() {
    ...
    this.mouseMoveHandler = this.ganttService.mouseMoveListener.subscribe((val) => {
      this.onMouseMove(val as number);
    });
    this.mouseEndHandler = this.ganttService.mouseEndListener.subscribe(this.onMouseEnd.bind(this));
    this.goToday();
  }

  onGanttBarMoveEnd (e: any) {}
  ...
  adjustScrollView(info: GanttTaskInfo) {
    if ((info.left as number) + (info.width as number) > this.scrollElement.scrollLeft + this.scrollElement.clientWidth) {
      this.scrollElement.scrollTo(this.scrollElement.scrollLeft + this.scaleStep, this.scrollElement.scrollTop);
    }
    if ((info.left as number) < this.scrollElement.scrollLeft) {
      this.scrollElement.scrollTo(this.scrollElement.scrollLeft - this.scaleStep, this.scrollElement.scrollTop);
    }
  }
